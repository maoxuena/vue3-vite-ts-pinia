import { DatavComponent, ComponentAttr } from '@/store/modules/types'
...
function calcResizeForCorner(
  ...
  if (direction[0] === 'l') {
    ...
    pos.width = Math.round((newCornerPoint.x - newDiagonalPoint.x) / scale)
  } else if (direction[0] === 'r') {
    pos.x = Math.round(newCornerPoint.x / scale)
    pos.width = Math.round((newDiagonalPoint.x - newCornerPoint.x) / scale)
  }

  if (direction[1] === 't') {
    pos.y = Math.round(newDiagonalPoint.y / scale)
    pos.height = Math.round((newCornerPoint.y - newDiagonalPoint.y) / scale)
  } else if (direction[1] === 'b') {
    pos.y = Math.round(newCornerPoint.y / scale)
    pos.height = Math.round((newDiagonalPoint.y - newCornerPoint.y) / scale)
  }
}
...
function calcResizeForEdge(
  ...
  if (dir === 't' || dir === 'b') {
    ...
    if (height > 0) {
      ...
      pos.height = Math.round(height / scale)
      pos.x = Math.round(center.x / scale - attr.width / 2)
  ...
  if (dir === 'l' || dir === 'r') {
    ...
    if (width > 0) {
      ...
      pos.width = Math.round(width / scale)
      pos.x = Math.round((center.x - width / 2) / scale)
      pos.y = Math.round(center.y / scale - attr.height / 2)
    }
  }
}
...
function calcResizeForNormal(
  ...
  if (dir === 't') {
    pos.height = Math.round(attr.height + (startPoint.y - curPositon.y) / scale)
    pos.y = Math.round(attr.y + (curPositon.y - startPoint.y) / scale)
  } else if (dir === 'rt') {
    pos.height = Math.round(attr.height + (startPoint.y - curPositon.y) / scale)
    pos.width = Math.round(attr.width + (curPositon.x - startPoint.x) / scale)
    pos.y = Math.round(attr.y + (curPositon.y - startPoint.y) / scale)
  } else if (dir === 'r') {
    pos.width = Math.round(attr.width + (curPositon.x - startPoint.x) / scale)
  } else if (dir === 'rb') {
    pos.height = Math.round(attr.height + (curPositon.y - startPoint.y) / scale)
    pos.width = Math.round(attr.width + (curPositon.x - startPoint.x) / scale)
  } else if (dir === 'b') {
    pos.height = Math.round(attr.height + (curPositon.y - startPoint.y) / scale)
  } else if (dir === 'lb') {
    pos.height = Math.round(attr.height + (curPositon.y - startPoint.y) / scale)
    pos.width = Math.round(attr.width + (startPoint.x - curPositon.x) / scale)
...
const setAttr = (
  ...
  if (resizeMode === 'stretch') {
    const center = { x: (attr.x + attr.width / 2) * scale, y: (attr.y + attr.height / 2) * scale }
    layoutRect = document.getElementById('screen-panel')!.getBoundingClientRect()
  ...
  const move = (e: MouseEvent) => {
    if (dir) {
      if (resizeMode === 'normal') {
        calcResizeForNormal(dir, attr, { x: ev.clientX, y: ev.clientY }, { x: e.clientX, y: e.clientY }, scale, pos)
      } else {
...
export const handleMove = (
  ...
  setAttr(ev, null, com, scale, grid, 'normal', moveCallback, upCallback)
}

export const handleRotate = (ev: MouseEvent, el: HTMLElement, com: DatavComponent, moveCallback: () => void, upCallback: () => void) => {
  // 获取元素中心点位置
  const rect = el.getBoundingClientRect()
  const centerX = rect.left + rect.width / 2
  const centerY = rect.top + rect.height / 2

  const startAngle = (Math.atan2(centerY - ev.clientY, centerX - ev.clientX) * 180) / Math.PI - com.attr.deg
